...
/// Failures from spawning or running [`run_chat`].
#[derive(Debug, thiserror::Error)]
pub enum ChatServiceError {
    /// `ChatConfig.enabled = false` or the `DAEJI_CHAT_DISABLED` env var was set.
    #[error("chat is disabled in config")]
    Disabled,
    /// Configured `me_seed` could not be parsed.
    #[error("invalid me_seed encoding")]
    InvalidSeed,
    /// One of the configured bootstrappers did not parse as `seed@host:port`.
    #[error("invalid bootstrapper spec: {0}")]
    InvalidBootstrapper(String),
    /// A seeded job's `room_key_hex` was not 64 hex characters (32 bytes).
    #[error("invalid room key: must be 32 bytes of hex (64 chars)")]
    InvalidRoomKey,
    /// A seeded job's `job_id` was not a `u64` decimal string.
    #[error("invalid seed_job.job_id: must be a u64 decimal string ({0})")]
    InvalidSeedJobId(String),
    /// Registry file IO failed (load or save).
