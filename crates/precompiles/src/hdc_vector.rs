...
//!
//! Vendored from `roko-primitives/src/hdc.rs` (MIT OR Apache-2.0). Upstream:
//! `https://github.com/Nunchi-trade/roko/tree/main/crates/roko-primitives`.
//! `random()` removed (chain precompiles must be deterministic).
...
impl HdcVector {
    ...
    /// Returns a deterministic pseudo-random vector from a u64 seed.
    /// (Removed `random()` method that used `uuid::Uuid::new_v4()` —
    ...
    // (rkyv-archived `similarity_archived` removed — daeji doesn't pull rkyv as a dep.
    // Re-introduce if mmap'd zero-copy reads ever get wired up.)
}
...
mod tests {
    ...
    fn hdc_bind_involution() {
        let a = HdcVector::random_from_seed(0xdead_beef_cafe_babe);
        let b = HdcVector::random_from_seed(0x5eed_5eed_5eed_5eed);
    ...
    fn hdc_similarity_self() {
        let vector = HdcVector::random_from_seed(42);
    ...
    fn hdc_bytes_roundtrip() {
        let v = HdcVector::random_from_seed(0x1234_5678_90ab_cdef);
