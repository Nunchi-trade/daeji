...
//!
//! Vendored from `roko/apps/mirage-rs/src/precompiles/hdc.rs` (MIT OR
//! Apache-2.0). The `PrecompileProvider` trait shape in revm 38 is identical
//! to revm 36's, so the dispatcher logic ports without changes; only imports
//! are re-pointed to local crate modules.
//!
//! Solidity surface: see `~/contracts-core/packages/agents/src/IHDCPrecompile.sol`.
//!
//! # ABI
//!
//! Inputs and outputs follow standard Solidity ABI encoding. `HdcVector` is
//! serialised as a dynamic `bytes` of exactly 1,280 bytes (10,240 bits).
//! `InsightId` is `bytes16`. `f32` similarity / weight / score values are
//! scaled by 1e6 and returned as `uint32`.
//!
//! # State
//!
