...
impl QmdbLedger {
    ...
    pub async fn init_with_genesis(
        ...
        let root_provider_ctx = context.child("root_provider");
        let backend = CommonwareBackend::open(context, config.clone()).await?;
        ...
        let root_provider = CommonwareRootProvider::new(root_provider_ctx, config);
