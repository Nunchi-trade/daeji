...
export class BlockPoller {
    ...
    async pollBlock() {
        try {
            ...
            // No new block: skip
            if (currentNumber <= this.lastBlockNumber)
                return;
            ...
            const effectiveStart = currentNumber - startBlock > 3n ? currentNumber - 2n : startBlock;
     ...
     */
    async fetchAndEmitBlock(blockNumber) {
    ...
    async pollFees() {
        try {
            ...
            if (!history || !history.baseFeePerGas)
                return;
