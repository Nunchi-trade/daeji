...
function createMockContext() {
  ...
  return ctx as unknown as CanvasRenderingContext2D & { fills: typeof fills };
}

describe('generateHashArt', () => {
  it('produces deterministic output for the same hash', () => {
    const hash =
      '0x6d1fb175dce4b6795ca4dc32a19a0d6b6ab5c25b8e48ce49aaee6c28d5d5a0c1' as `0x${string}`;
  ...
  it('always renders exactly 8x8 grid cells max (plus background)', () => {
    ...
    expect(ctx.fills[0]).toEqual(
      expect.objectContaining({ x: 0, y: 0, w: 64, h: 64 }),
    );
