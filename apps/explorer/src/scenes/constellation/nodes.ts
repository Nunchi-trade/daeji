...
const COLOR_DORMANT: [number, number, number, number] = [0.227, 0.188, 0.227, 0.15]; // #3a303a
const COLOR_ACTIVE: [number, number, number, number] = [0.863, 0.647, 0.741, 1.0]; // #dca5bd rose-glow
const COLOR_CONTRACT: [number, number, number, number] = [0.667, 0.439, 0.533, 1.0]; // #aa7088 rose
export const COLOR_HIGH_VALUE: [number, number, number, number] = [0.847, 0.784, 0.627, 1.0]; // #d8c8a0 bone-bright
...
export interface AddressNode {
  ...
  x: number;
  y: number;
  z: number; // subtle parallax depth
  isContract: boolean;

  // Visual state (mutated per frame)
  r: number;
  g: number;
  b: number;
  a: number;
  size: number; // point size in pixels
  pulsePhase: number; // 0..TAU
  lastActiveTime: number; // performance.now()
  txCount: number;
}

// ── Deterministic position from address ──────────────────────

function parseHex4(hex: string, offset: number): number {
 ...
 */
export function addressToPosition(address: Hex): { x: number; y: number; z: number } {
...
export class AddressBook {
  private nodes = new Map<string, AddressNode>();
  private list: AddressNode[] = [];
  private dirty = false;
  ...
  getOrCreate(address: Hex): AddressNode {
    ...
    const pos = addressToPosition(address as Hex);
    node = {
  ...
  get(address: Hex): AddressNode | undefined {
    return this.nodes.get(address.toLowerCase());
  }

  markActive(node: AddressNode): void {
  ...
  markContract(node: AddressNode): void {
  ...
  breathScale(node: AddressNode, time: number): number {
    // 5s cycle, ±2% scale
    return 1.0 + Math.sin(time * 1.2566 + node.pulsePhase) * 0.02;
  }

  /** Pulse scale for active nodes. */
  pulseScale(node: AddressNode): number {
    if (node.a < 0.5) return 1.0;
