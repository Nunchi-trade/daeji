...
const ARC_COLOR: [number, number, number] = [0.847, 0.784, 0.627]; // #d8c8a0
...
export class ParticleSystem {
   ...
   */
  spawnArcParticles(arc: ArcData, particleCount: number): void {
  ...
  spawnFireflies(centerX: number, centerY: number, radius: number, count: number): void {
    for (let i = 0; i < count; i++) {
      if (this.count >= this.maxParticles) return;
