...
import type { Hex, ChainBlock, ChainTransaction, ChainReceipt } from '@/data/types';
...
function ConstellationCanvas() {
  ...
  const init = useCallback((canvas: HTMLCanvasElement) => {
    ...
    const drawPoints = regl({
      ...
      uniforms: {
        ...
        uZoom: () => camera.zoom,
        uResolution: (ctx: { viewportWidth: number; viewportHeight: number }) => [
          ctx.viewportWidth,
          ctx.viewportHeight,
        ],
      },
    ...
    const drawLines = regl({
      ...
      uniforms: {
        ...
        uColor: regl.prop<{ uColor: [number, number, number, number] }, 'uColor'>('uColor'),
      },
    ...
    const onBlock = (block: ChainBlock) => {
    ...
    const onTxConfirmed = (ev: { hash: Hex; receipt: ChainReceipt }) => {
    ...
    const onTxPending = (tx: ChainTransaction) => {
      const fromNode = addressBook.getOrCreate(tx.from);
      if (tx.to) {
        const toNode = addressBook.getOrCreate(tx.to);
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, overflow: 'hidden', background: '#060608' }}>
      ...
      />
      <noscript>
        <div style={{ color: '#6a5a68', textAlign: 'center', paddingTop: '40vh' }}>
          WebGL2 is required for the constellation view.
        </div>
      </noscript>
    </div>
  );
