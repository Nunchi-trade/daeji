...
export function ValidatorNode({ index, angle, radius, sceneStateRef }) {
    const meshRef = useRef(null);
    const glowRef = useRef(null);
    const position = useMemo(() => [Math.cos(angle) * radius, 0, Math.sin(angle) * radius], [angle, radius]);
    ...
    useFrame((_state, delta) => {
        ...
        mat.emissiveIntensity = THREE.MathUtils.lerp(mat.emissiveIntensity, visualState === 'idle' ? 0.1 : 0.5, 4 * delta);
        // Smooth scale
        ...
        glowMat.opacity = THREE.MathUtils.lerp(glowMat.opacity, glowVisible ? 0.3 : 0, 4 * delta);
    ...
    return (_jsxs("group", { position: position, children: [_jsxs("mesh", { ref: meshRef, rotation: [0, angle, 0], children: [_jsx("octahedronGeometry", { args: [0.5, 0] }), _jsx("meshStandardMaterial", { color: COLORS.idle, emissive: COLORS.idle, emissiveIntensity: 0.1, transparent: true, opacity: 0.9, roughness: 0.3, metalness: 0.6 })] }), _jsxs("mesh", { ref: glowRef, rotation: [-Math.PI / 2, 0, 0], children: [_jsx("ringGeometry", { args: [0.6, 1.0, 32] }), _jsx("meshBasicMaterial", { color: COLORS.voted, transparent: true, opacity: 0, side: THREE.DoubleSide, depthWrite: false, blending: THREE.AdditiveBlending })] }), _jsx("sprite", { position: [0, 1.2, 0], scale: [1.5, 0.4, 1], children: _jsx("spriteMaterial", { map: labelTexture, transparent: true, opacity: 0.7, depthTest: false }) })] }));
}
