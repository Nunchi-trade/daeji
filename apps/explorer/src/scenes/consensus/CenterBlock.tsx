...
const CENTER_SCALE: Record<CenterBlockState, number> = {
  empty: 0.3,
  wireframe: 0.6,
  filling: 0.8,
  solid: 1.0,
...
export function CenterBlock({ sceneStateRef }: Props) {
  ...
  const fragmentVelocities = useRef(
    Array.from({ length: 8 }, () =>
      new THREE.Vector3(
        (Math.random() - 0.5) * 4,
        Math.random() * 2,
  ...
  const prevCenterState = useRef<CenterBlockState>('empty');
  ...
  useFrame((_state, delta) => {
    ...
    if (
      centerState === 'crystallized' &&
      prevCenterState.current !== 'crystallized'
    ) {
      if (flash) {
        (flash.material as THREE.MeshBasicMaterial).opacity = 0.8;
      }
    }
    ...
    const wireVisible =
      centerState === 'wireframe' || centerState === 'filling';
    ...
    mesh.scale.setScalar(
      THREE.MathUtils.lerp(currentScale, targetScale, 6 * delta),
    );
    ...
    if (centerState === 'shattering') {
      ...
      for (let i = 0; i < fragmentRefs.current.length; i++) {
        ...
        (frag.material as THREE.MeshStandardMaterial).opacity = Math.max(
          0,
          1 - elapsed * 2,
        );
      ...
      for (const frag of fragmentRefs.current) {
        if (frag) {
          ...
          (frag.material as THREE.MeshStandardMaterial).opacity = 0;
        }
      }
    }
  });
  ...
  return (
    <group position={[0, 0, 0]}>
      ...
      {/* Wireframe overlay */}
      <lineSegments ref={wireRef} geometry={_edgesGeo}>
        <lineBasicMaterial
          color="#9494b4"
          transparent
          opacity={0}
