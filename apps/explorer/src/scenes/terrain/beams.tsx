 ...
 */
function txToBeamOffset(txHash: string): [number, number] {
...
function txToBeamHeight(tx: ChainTransaction, gasLimit: bigint): number {
  if (gasLimit === 0n) return 0.5;
...
export function TransactionBeams({ ring }: Props) {
  ...
  const beamPool = useRef<THREE.Mesh[]>([]);
  ...
  const beamGeometry = useMemo(
    () => new THREE.PlaneGeometry(BEAM_WIDTH, 1, 1, 1),
    [],
  );
  ...
  useFrame(() => {
    ...
    for (const slot of ring.getSlots()) {
      if (!slot.active || slot.transactions.length === 0) continue;

      for (const tx of slot.transactions) {
        // Check if beam already exists for this tx
        const existing = beamPool.current.find(
          (m) => m.userData.txHash === tx.hash,
        );
    ...
    for (const mesh of beamPool.current) {
      ...
      const tileId = mesh.userData.tileId as number;
      const slot = ring.getSlots().find((s) => s.id === tileId);
      if (slot && slot.active) {
        const [, offsetZ] = mesh.userData.txHash
          ? txToBeamOffset(mesh.userData.txHash as string)
          : [0, 0];
