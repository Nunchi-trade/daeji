...
function bytesToNorm(bytes, start, end) {
    let val = 0;
    for (let i = start; i < end; i++)
        val = (val * 256 + bytes[i]) >>> 0;
...
function seededPerlin2D(x, y, perm) {
...
// ---- Erosion ----------------------------------------------------------------
function thermalErosion(heights, size, talusAngle) {
    const transfer = 0.3;
...
function hydraulicErosion(heights, size, rng) {
...
export function extractParams(hash) {
 ...
 */
export function hashToHeightmap(hash, gridSize = 32) {
    const bytes = hexToBytes(hash);
...
export function heightToColor(height, hueShift) {
