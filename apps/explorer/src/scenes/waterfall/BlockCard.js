...
export class BlockCard {
    ...
    animStartTime;
    static FALL_DURATION = 400; // ms
    static MAX_VISIBLE = 50;
    constructor(block) {
    ...
    draw(ctx, x, y, width) {
        ...
        if (h >= 14) {
            ...
            ctx.fillText(Number(this.blockNumber).toLocaleString(), x + accentWidth + 6, y + h / 2);
        }
        // Tx count (right side)
        if (this.txCount > 0 && h >= 14) {
            ctx.textAlign = 'right';
            ctx.fillStyle = '#d8c8a0'; // --rd-bone-bright
            ctx.fillText(`${this.txCount} tx`, x + width - 6, y + h / 2);
        }
        // Bottom border
