...
import { WaterfallTooltip } from './tooltip';
...
function WaterfallScene() {
    ...
    const render = useCallback(() => {
        ...
        const lastVisible = Math.min(blocks.length - 1, Math.ceil((scrollY + height) / rowHeight));
    ...
    const onNewBlock = useCallback((block) => {
        const blocks = blocksRef.current;
    ...
    const onWheel = useCallback((e) => {
        e.preventDefault();
        const rowHeight = BAR_HEIGHT + BAR_GAP;
        const maxScroll = Math.max(0, blocksRef.current.length * rowHeight -
    ...
    const onTouchMove = useCallback((e) => {
        ...
        const maxScroll = Math.max(0, blocksRef.current.length * rowHeight -
            (canvasRef.current?.height ?? 0) / dprRef.current);
        targetScrollRef.current = Math.max(0, Math.min(maxScroll, touchScrollStartRef.current + deltaY));
    ...
    return (_jsxs("div", { style: { display: 'flex', flexDirection: 'column', height: '100%' }, children: [_jsxs("div", { ref: containerRef, style: {
                    width: '100%',
                    flex: 1,
                    minHeight: 0,
                    position: 'relative',
                    overflow: 'hidden',
                            ...
                            height: '100%',
                            cursor: 'pointer',
                        } }), _jsx(WaterfallTooltip, { setRef: tooltipSetRef })] }), _jsx(BlockPulse, {}), _jsx(GasWaveform, {})] }));
}
