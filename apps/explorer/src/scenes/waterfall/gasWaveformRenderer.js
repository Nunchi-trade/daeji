...
export class GasWaveformRenderer {
    ...
    hitTest(clientX) {
        ...
        if (index < 0 || index >= s.gasUsedHistory.length)
            return null;
        return {
    ...
    redraw() {
        ...
        const h = this.height;
        const s = this.state;
        if (w === 0 || s.gasUsedHistory.length < 2)
            return;
        ...
        // Compute points
        const points = s.gasUsedHistory.map((gas, i) => {
        ...
        for (let i = 0; i < s.blockNumbers.length; i += step) {
            const x = marginLeft + (i / (POINT_COUNT - 1)) * plotW;
            ctx.fillText(Number(s.blockNumbers[i]).toLocaleString(), x, marginTop + plotH + 4);
        }
    }
}
