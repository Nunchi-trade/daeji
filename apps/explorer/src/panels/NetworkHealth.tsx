...
function describeArc(startDeg: number, sweepDeg: number, r: number, cx: number, cy: number): string {
...
export function NetworkHealth() {
  ...
  const score = useMemo(() => {
    ...
    if (recentOrder.length >= 2) {
      ...
      for (const num of recentOrder) {
        ...
        if (b && b._arrivalTime) arrivals.push(b._arrivalTime);
      }
  ...
  const arcColor =
    score > 70 ? '#7a8a78' : score > 40 ? '#c89a68' : '#cc5555';
